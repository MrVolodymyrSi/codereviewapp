# GitHub-style Commenting Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the current gutter comment system with GitHub PR-style mechanics: line number column click/drag to select a range, inline comment form with range label, and posted comments rendered as bordered cards with a code snippet header, author row with relative timestamp, and inline Edit/Delete via a `···` menu.

**Architecture:** Four files change. `Comment` type gains `lineStart`/`lineEnd` (replacing `line`). `useComments` updates `addComment` and adds legacy migration in `hydrateComments`. `useGutterComments` is a full rewrite covering selection, DOM builders, and styles. `CodePane` wires the new API.

**Tech Stack:** Vue 3, TypeScript, Monaco Editor (deltaDecorations, changeViewZones, onMouseDown/Move/Leave), vanilla DOM
...
| File | Change |
|---|---|
| `src/types/comment.ts` | `line` → `lineStart` + `lineEnd` |
| `src/composables/useComments.ts` | `addComment` signature + `hydrateComments` migration |
| `src/composables/useGutterComments.ts` | Full rewrite |
| `src/components/CodePane.vue` | Wire new API |

No other files change. `MonacoEditor.vue` already has `glyphMargin: true` — leave it as-is.
...
- [ ] **Verify TypeScript errors appear in useComments.ts and useGutterComments.ts** (expected — fixed in Tasks 2 and 3)

- [ ] **Commit**

```bash
git add src/types/comment.ts
git commit -m "feat: replace Comment.line with lineStart/lineEnd"
```

---

### Task 2: Update useComments composable
...
```bash
git add src/composables/useComments.ts
git commit -m "feat: update addComment to accept lineStart/lineEnd, migrate legacy line field in hydrateComments"
```

---

### Task 3: Rewrite useGutterComments
...
git commit -m "feat: rewrite useGutterComments with GitHub-style line range selection and comment cards"
```

---

### Task 4: Update CodePane.vue

**Files:**
- Modify: `src/components/CodePane.vue`

- [ ] **Replace the `<script setup>` block** (lines 1–86 in the current file — keep `<template>` and `<style>` unchanged)
...
- [ ] **Verify no TypeScript errors** — run:

```bash
cd /Users/volodymyrsinievych/1010s/claude-code-repo/codereview && npx tsc --noEmit
```
...
git commit -m "feat: wire CodePane to new GitHub-style commenting API"
```
...
- [ ] **Hover over line numbers** — a blue `+` should appear in the glyph margin to the left

- [ ] **Click a single line number** — mousedown on the line number should trigger `onRangeSelect(N, N)` → form zone appears below that line with header "Commenting on line N". Type a comment and click Add comment. Verify the card appears with the code snippet, author, timestamp, and `···` menu.

- [ ] **Click + drag across multiple line numbers** — lines should highlight with a blue tint and left border during drag. On mouseup, form zone should appear with "Commenting on lines N–M".

- [ ] **`···` menu → Delete** — comment card should be removed.

- [ ] **`···` menu → Edit** — textarea pre-filled with existing text should appear. Modify text and click Save. Card body should update. Cancel should restore original text.

- [ ] **Switch file tab** — any open form zone should close.

- [ ] **Reload with existing persisted comments** — comments with legacy `line` field (no `lineStart`/`lineEnd`) should hydrate correctly, appearing at the correct line.
